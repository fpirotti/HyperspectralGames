  ...
  column(
    ...
    tabsetPanel(
      ...
      tabPanel(
        ...
        fluidRow(
        #   column(
        #   2, style="margin-top:24px;",
        #   shinyWidgets::actionBttn(
        #     inputId = "calcGraphGV",
        #     label = translations$calcola[[lang]] ,
        #     icon =  icon("gear"),
        #     color = "success",
        #     style = "fill",
        #     size = "sm"
        #   )
        # ),
          column(6, title="Dimensioni della finestra che verrà ritagliata intorno al punto selezionato",
               sliderInput("dimGV", "Dimensione finestra", min=5, step=2, max=25, value=11)
               ),
          column(7,  
               div(style="text-align:justify;", "<b>Clicca</b> sull'immagine a sinistra per prendere una piccola finestra di NxN pixel (dove N = dimensione finestra impostata a sinistra) e disegnare X grafici interattivi con i valori di grigio - dove X=numero di bande dell'immagine")
           )
        ),

        plotlyOutput("graphGVtiles" , height = 1000) #,
...
observeEvent(input$plot_click, {
 ...
 runjs( sprintf("  $(\"#finestraGV\").css({ 'top':'%spx', 'left':'%spx', 'width':'%spx', 'height':'%spx'  }); ",  row1-2, col1-2, size+4, size+4 ) )
