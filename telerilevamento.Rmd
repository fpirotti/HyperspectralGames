...
translations<-list(
  ...
  "docSpectral" = list("ENG"="<b>Clicks on left image will plot spectral signatures. Bands of Sentinel 2 are plotted - hover with mouse over bottom squares for description of bands</b>",
                       "ITA"="<b>I click del mouse disegnano la forma spettrale. Le fasce rappresentano le bande di sensibilità del Sentinel 2 - posiziona il mouse sopra il quadrato nero in basso per una descrizione della banda</b>"),
...
``` 
<p>SI CONSIGLIA DI VISUALIZZARE LA PAGINA INGRANDENDO </p>
    ...
    tabsetPanel(
      ...
      tabPanel(
        ...
        fluidRow(
          ...
          column(8,  
               div(style="text-align:justify;", HTML(translations$docGV[[lang]]) )
           )
        ),
...
shiny::observeEvent(input$indexGo, {
  ...
  output$indexPlot <- renderPlot( plot(1:12)   )
  
  output$graphGVtiles <- renderPlot( plot(1:12)   )
...
observeEvent(input$plotIndex_hover, {
  ...
  runjs(sprintf( "Value: $('#indexValue').html('%s')", va  ) )
...
observeEvent(input$pannelli, {
 ...
 runjs( sprintf("  $(\"#finestraGV\").css({ 'visibility':'%s' }); ",
...
observeEvent({ input$plot_click
  ...
  req(input$plot_click, input$dimGV)
  
 if( input$pannelli=="GV.panel" ){ 
    # if( as.integer(input$zoomLens)==0 ){
    #   runjs("highlightWidget( $(\"#zoomLens\"));")
    #   showNotification("Seleziona una dimensione della lente per visualizzare una finestra sul puntatore mentre ci si muove sull'immagine a sinistra dal quale prendere i valori di grigio")
    #   return(NULL)
    # }
...
observeEvent(input$saveTrain, {
  ...
  img.file.sigs<-img.file
  raster::extension(img.file.sigs)<-"rds"
  if(file.exists(img.file.sigs)) runjs("alert('Attenzione, sovrascrivo il file se esistente');")
  saveRDS(isolate(react$train.table), img.file.sigs)
  shinyWidgets::alert("asdfasd")
})
...
output$trainingTable <- DT::renderDataTable({
 
  req(react$train.table)
