...
table.dataTable.hover tbody tr:hover, table.dataTable.display tbody tr:hover {
  background-color: pink !important; cursor:pointer;
}
...
```{r, context="server", message=FALSE }
  ...
  column(
    ...
    tabsetPanel(
      ...
      tabPanel("Training POI",   value = "classificazione.panel",
               ...
               fluidRow(
                 column(
                   ...
                   div( style="display:flex;gap:5px;", 
                    div( 
                     title = "Elimina le righe selezionate",
                     shinyWidgets::actionBttn(
                       "delTrain",
                       label = "Elimina",
                       size = "sm",
                  ...
                  div( 
                     title = "Estrai firme spettrali dai POI creati aggregandoli per classe", 
                     shinyWidgets::actionBttn(
                       "plotTrain",
                       label = "Calcola Firme Spettrali",
                       size = "sm",
                       block = F,
                       style = "fill",
                       shiny::icon("pen")
       ...
       shinycssloaders::withSpinner( plotlyOutput("plotTrainSignature" , height = 500), type = 1, hide.ui = F ),
       shinycssloaders::withSpinner(  imageOutput("plot4"  ), type = 1, hide.ui = F ),
       shinycssloaders::withSpinner( imageOutput("plot5"  ), type = 1, hide.ui = F )
       )
    )
  )
...
observeEvent(input$plotTrain, {
        ...
        ss[i,] <-
          as.numeric(
            gdalUtils::gdallocationinfo(
              img.file, geoloc=T, 
              react$train.table[i, "East"]  ,
              react$train.table[i, "North"],
              valonly = T
            )
          )
      }
  ...
  output$plotTrainSignature <- plotly::renderPlotly ({
      ggplot(df.with.ss4,
             aes(
               x = Wavelength,
               y = Q50,
      ...
      ggtitle("Spectral signatures of POIs - Q50 and IQR") +
      theme_bw() 
  })  
...
output$trainingTable <- DT::renderDT({
  ...
  " var index = table.row(this); var colIndex = table.columns().header().toArray().map(x => x.innerText).indexOf(\"\");  ", 
  " if(index!==undefined) {  Shiny.setInputValue('POItableRowHover', index.data()[colIndex], {priority: 'event'}); }",
  "});"
  )
...
observeEvent(input$classTrain, {
  
  req(input$classTrain, react$train.table)
 ...
 # saveRDS(react$train.table, "s2.train.table.rds")
  ...
  if (is.na(ss[1,1])) {
    shinyjs::alert("Signature matrix is NaN!")
    return(NULL)
  }
 ...
 setProgress(value = 0.7 ,
                ...
                rasters.rminer[[ model.names[[ "KNN" ]]    ]] <- raster::predict(  myImage, models.rminer[["KNN"]] )
                 ...
                 setProgress(value = 0.89, detail = "Creo grafico...")
                  ...
                  img.file.out<-img.file
                  raster::extension(img.file.out)<-sprintf("%s_rminer.tif", fs::path_sanitize(input$cookie.username, "_") )
                  rb.rminer <- raster::brick(rasters.rminer)
                  writeRaster(rb.miner, img.file.out)
