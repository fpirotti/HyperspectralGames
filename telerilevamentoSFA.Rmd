...
fluidRow(  
  ...
  column( 6,    style="margin:0;  !important; margin-bottom:10px;",     tabsetPanel(
        tabPanel("Firme Spettrali", 
         ...
         plotlyOutput("graph1" ) ),
        tabPanel("Indici",  
                 fluidRow( column(5,  style="margin:0;",    shiny::textInput("indexIn", "Formula") ),
                           column(5, style="margin:0;",  shinyWidgets::pickerInput("indexChoose", "Precaricati", 
                                                     choices = list("", 
                                                                    "Aerosol free vegetation index 1600"="B9-0.66*B11/(B9+0.6611)", 
                                                                    "Aerosol free vegetation index 2100"="B9-0.5*B12/(B9+0.5612)", 
                                                                    "Atmospherically Resistant Vegetation Index 2"="-0.18+1.17*(B9-B5)/(B9+B5)", 
                                                                    "NDVI"="(B8A-B4)/(B8A+B4)" ) ) ),
                           column(2, style="margin:0; margin-top:20px;", shinyWidgets::actionBttn(
                                                              inputId = "indexGo",
                                                              label=NULL, icon = 
                                                              icon("pencil"),
                                                              color = "success",
         ...
         shinyWidgets::addSpinner(imageOutput("indexPlot", width = ncol(myImage)+80, height= nrow(myImage)+40 ) )
                 ), 
...
shiny::observeEvent(input$indexGo, {
  req(input$indexChoose, input$indexGo)
  eq<-gsub("B([1-9A]{1,2})", "myImage[['B\\1']]", input$indexChoose)
  out.raster<-eval(parse(text=eq)) #ic
  output$indexPlot <- renderPlot({  rasterVis::levelplot(out.raster, par.settings = RdBuTheme, margin=F, xlab=NULL, ylab=NULL, scales=list(draw=FALSE) ) })
  
}, ignoreInit = T  )

shiny::observeEvent(input$indexChoose, {
   req(input$indexChoose)
   updateTextInput(session=session, inputId = "indexIn", value = input$indexChoose )
}, ignoreInit = T  )
