...
fluidRow(
 ...
 fluidRow(
 column(width=12, shinyWidgets::addSpinner( plotOutput("plot4") ) ) 
 ) 
...
withProgress(message="Getting signatures",
             detail = sprintf("%s trainers", nrow(react$train.table)),
             value=0, min=0, max=nrow(react$train.table),  { 
...
model.names<-list(RF="Random Forest",
  ...
  models<-list()
  setProgress( value = 0.21 , detail = sprintf("Defining model with '%s'.", model.names[["RF"]] ) )
  ...
  setProgress( value = 0.21 , detail = sprintf("Defining model with '%s'.", model.names[["NB"]] ) )
  ...
  setProgress( value = 0.21 , detail = sprintf("Defining model with '%s'.", model.names[["GMB"]] ) )
  models[["GMB"]]  <- h2o.gbm(x = x,
                  ...
                  seed = 1)
  ### Deep Learning Model
  setProgress( value = 0.21 , detail = sprintf("Defining model with '%s'.", model.names[["DL"]] ) )
 ...
 for(i in names(models)){ 
    ...
    setProgress( value = 0.5 + length(rasters)/10 , detail = sprintf("Predicting with %s", model.names[[i]] ) )
